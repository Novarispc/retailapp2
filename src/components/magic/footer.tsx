...
export async function Footer() {
  ...
  const social = "grid h-9 w-9 place-items-center rounded-full border border-[var(--border-strong)] text-muted transition-colors hover:border-[var(--accent)] hover:text-[var(--accent)]";
  ...
  return (
    <footer className="mt-24 border-t border-[var(--border)] px-6 py-14">
      <div className="mx-auto grid max-w-7xl gap-10 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <div className="lg:col-span-2">
          <Link href="/" className="mb-4 inline-flex">
            <Logo className="[&_span:last-child]:text-[1.5rem]" />
          </Link>
          ...
          <div className="mt-5 flex gap-3">
            {instagramUrl && (
              <a href={instagramUrl} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className={social}>
                <InstagramIcon className="h-4 w-4" />
              </a>
            )}
            {facebookUrl && (
              <a href={facebookUrl} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className={social}>
                <FacebookIcon className="h-4 w-4" />
              </a>
            )}
            {linkedinUrl && (
              <a href={linkedinUrl} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className={social}>
        ...
        <div>
          <h4 className="font-display mb-3 text-sm font-bold uppercase tracking-wide">{t("shop")}</h4>
        ...
        <div>
          <h4 className="font-display mb-3 text-sm font-bold uppercase tracking-wide">{t("account")}</h4>
          <ul className="space-y-2">
        ...
        <div>
          <h4 className="font-display mb-3 text-sm font-bold uppercase tracking-wide">{t("company")}</h4>
          <ul className="space-y-2">
      ...
      <div className="mx-auto mt-12 flex max-w-7xl flex-col gap-4 border-t border-[var(--border)] pt-6 text-xs text-muted sm:flex-row sm:items-center sm:justify-between">
        <span>© {new Date().getFullYear()} {footerName}. All rights reserved.</span>
        <span className="font-display flex flex-wrap items-center gap-x-3 gap-y-1 uppercase tracking-wide">
          {["360", "BDM", "DSC", "EM", "GOWIN", "SS"].map((b) => <span key={b}>{b}</span>)}
        </span>
      </div>
    </footer>
  );
